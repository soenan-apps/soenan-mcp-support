...
ToolCaller: TypeAlias = Callable[[str, Mapping[str, object]], ToolResult]
...
def upload_file(
    ...
    *,
    ...
    try:
        begin_arguments: dict[str, object] = {
        ...
        if mix_version_id is not None:
            begin_arguments["mixVersionId"] = mix_version_id
        ...
        _call_tool(
            call_tool,
            "audaligo_put_file_upload_manifest",
            {
                "projectId": project_id,
                "uploadId": upload_id,
                "manifestJson": json.dumps(
                    plan.manifest(), separators=(",", ":"), ensure_ascii=True
                ),
            },
        )
        ...
        for chunk in plan.chunks:
            ...
            capability = _call_tool(
                call_tool,
                "audaligo_create_file_upload_chunk_capability",
                {
                    "projectId": project_id,
                    "uploadId": upload_id,
                    "objectId": upload_id,
        ...
        _call_tool(
            call_tool,
            "audaligo_complete_file_upload_chunks",
            {
                "projectId": project_id,
                "uploadId": upload_id,
                "objectId": upload_id,
                "chunkIndexes": [chunk.index for chunk in plan.chunks],
            },
        )
        ...
        if mix_version_id is not None:
            commit_arguments["mixVersionId"] = mix_version_id
        return _call_tool(call_tool, "audaligo_commit_file_upload", commit_arguments)
    ...
    *,
    ...
    if isinstance(destination, (str, os.PathLike)):
        return _download_to_path(
            call_tool, project_id, file_id, Path(destination), plan, timeouts, transport
        )
    _require_writer(destination)
    rollback = _append_rollback_position(destination)
    try:
        return _download_to_stream(
            call_tool, project_id, file_id, destination, plan, timeouts, transport
        )
    ...
    try:
        with os.fdopen(descriptor, "wb") as stream:
            count = _download_to_stream(
                call_tool, project_id, file_id, stream, plan, timeouts, transport
            )
            stream.flush()
...
def _download_to_stream(
    ...
    for chunk in plan.chunks:
        capability = _call_tool(
            call_tool,
            "audaligo_create_file_read_chunk_capability",
            {
                "projectId": project_id,
                "fileId": file_id,
...
def _call_tool(
    call_tool: ToolCaller, name: str, arguments: Mapping[str, object]
) -> ToolResult:
    try:
        ...
        # The external MCP client can expose credentials through its exception text.
        raise TransferError(f"{name} failed") from None
    if not isinstance(value, Mapping):
        raise TransferError(f"{name} returned an invalid result")
